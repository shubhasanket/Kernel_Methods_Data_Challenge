...
## Complete code, including all helper and preprocessing functions
## Runs the entire pipeline, can be time and resource intensive
...
SEARCH_RESULTS_PATH = os.path.join(BASE_DIR, "search_3layer_ckn_hog_upper.csv")
...
def cfg_to_key(cfg):
    return (
        f"l1={cfg['l1_filters']}|l2={cfg['l2_filters']}|l3={cfg['l3_filters']}|"
        f"m={cfg['nystrom_m']}|rff={cfg['rff_D']}|"
        f"lam={cfg['lam']}|temp={cfg['mkl_temp']}|seed={cfg['seed']}"
    )

def run_one_config(Xtr, ytr, Xva, yva, cfg):
    ...
    del builder, Ftr, Fva, Ztr, Zva, clf
    gc.collect()
...
def build_config_grid():
    ...
    l2_opts = [112, 128, 144]
    l3_opts = [128, 144, 160]
    nystrom_opts = [180, 220, 260]
    rff_opts = [256, 320, 384]
    lam_opts = [0.02, 0.03, 0.04]
    temp_opts = [1.5]
    seed_opts = [1]
...
def search_configs(Xtr, ytr, Xva, yva):
    configs = build_config_grid()

    if os.path.exists(SEARCH_RESULTS_PATH):
        results_df = pd.read_csv(SEARCH_RESULTS_PATH)
        print(f"Resuming search from: {SEARCH_RESULTS_PATH}")
    else:
        results_df = pd.DataFrame(columns=[
            "cfg_key", "l1_filters", "l2_filters", "l3_filters",
            "nystrom_m", "rff_D", "lam", "mkl_temp", "seed", "val_acc"
        ])
        print(f"Starting fresh search. Saving to: {SEARCH_RESULTS_PATH}")

    done = set(results_df["cfg_key"].tolist()) if len(results_df) > 0 else set()

    best_acc = -1.0
    best_cfg = None

    if len(results_df) > 0:
        best_idx = results_df["val_acc"].astype(float).idxmax()
        best_acc = float(results_df.loc[best_idx, "val_acc"])
        best_cfg = {
            "l1_filters": int(results_df.loc[best_idx, "l1_filters"]),
            "l2_filters": int(results_df.loc[best_idx, "l2_filters"]),
            "l3_filters": int(results_df.loc[best_idx, "l3_filters"]),
            "nystrom_m": int(results_df.loc[best_idx, "nystrom_m"]),
            "rff_D": int(results_df.loc[best_idx, "rff_D"]),
            "lam": float(results_df.loc[best_idx, "lam"]),
            "mkl_temp": float(results_df.loc[best_idx, "mkl_temp"]),
            "seed": int(results_df.loc[best_idx, "seed"]),
        }
        print("Best checkpointed config so far:", best_cfg)
        print("Best checkpointed val_acc:", best_acc)

    total = len(configs)
    for idx, cfg in enumerate(configs, 1):
        key = cfg_to_key(cfg)
        if key in done:
            print(f"[{idx}/{total}] already done: {key}")
            continue

        print(f"\n[{idx}/{total}] Testing {cfg}")
        acc, weights = run_one_config(Xtr, ytr, Xva, yva, cfg)
        print(f"Validation accuracy: {acc:.4f}")
        print("MKL weights:", weights)
        ...
        if acc > best_acc:
            best_acc = acc
            best_cfg = cfg
            print("New best found:", best_cfg, "acc=", best_acc)

        gc.collect()

    print("\nBest validation accuracy:", best_acc)
    print("Best config:", best_cfg)
    return best_cfg, best_acc, results_df

# ensembling routine
def top_configs_from_results(results_df, k=3):
    rs = results_df.sort_values(by="val_acc", ascending=False).reset_index(drop=True)
    out = []
    seen = set()
    for _, row in rs.iterrows():
        cfg = {
            "l1_filters": int(row["l1_filters"]),
            "l2_filters": int(row["l2_filters"]),
            "l3_filters": int(row["l3_filters"]),
            "nystrom_m": int(row["nystrom_m"]),
            "rff_D": int(row["rff_D"]),
            "lam": float(row["lam"]),
            "mkl_temp": float(row["mkl_temp"]),
            "seed": int(row["seed"]),
        }
        key = cfg_to_key(cfg)
        if key not in seen:
...
def fit_predict_single_config(Xtr, ytr, Xte, cfg):
    ...
    del builder, Ftr, Fte, Ztr, Zte, clf
    gc.collect()
    return scores
...
def main():
    ...
    print("Shapes:", Xtr.shape, Xte.shape, Ytr.shape)
    ...
    best_cfg, best_acc, results_df = search_configs(X_train, y_train, X_val, y_val)

    top_cfgs = top_configs_from_results(results_df, k=3)
    print("\nTop configs:")
    for c in top_cfgs:
        print(c)
    ...
    try:
        ...
        for i, cfg in enumerate(top_cfgs):
            print(f"\nValidation ensemble model {i+1}/{len(top_cfgs)}")
        ...
        val_ens_acc = np.mean(val_pred == y_val)
        print("\nValidation best single:", best_acc)
        print("Validation ensemble   :", val_ens_acc)
    except Exception as e:
        print("Ensemble validation skipped due to:", repr(e))
        val_ens_acc = None

    if val_ens_acc is not None and val_ens_acc > best_acc:
        print("\nUsing ensemble for final prediction.")
        Yte_pred = ensemble_predict(Xtr, Ytr, Xte, top_cfgs)
    else:
        print("\nUsing best single config for final prediction.")
        scores = fit_predict_single_config(Xtr, Ytr, Xte, best_cfg)
        Yte_pred = np.argmax(scores, axis=1)
    ...
    print("Search checkpoint:", SEARCH_RESULTS_PATH)
